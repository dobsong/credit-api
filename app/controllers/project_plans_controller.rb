class ProjectPlansController < ApplicationController
  ...
  private
    ...
    def set_project_plan
      @project_plan = ProjectPlan.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def project_plan_params
      # todo - get rid of "default_user" once auth is in place
      current_user = "default_user"
      params.require(:project_plan).permit(:title, :previous_engagement, :has_started, :vision, :laymans_summary, :stakeholder_analysis, :approach, :data, :ethics, :platform, :support_materials, :costings).merge(user: current_user)
    end
end
